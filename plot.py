...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(trainvalset, batch_size=batchsize, drop_last=False, shuffle=False)
...
class segmentation_net(nn.Module):
    ...
    def forward(self, x):
        ...
        y_npy = res.cpu().numpy()
        np.save('plots/x_'+str(self.cnt)+'.npy', x_npy)
        np.save('plots/y_'+str(self.cnt)+'.npy', y_npy)
        self.cnt += 1
        res = self.pool1(res)
